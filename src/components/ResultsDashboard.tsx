    ...
    <div className="py-16 px-4 pb-24">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Your Security Results</h2>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {Object.entries(riskCounts).map(([risk, count]) => (
              <div key={risk} className="text-center p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-sm text-gray-600 capitalize">{risk} Risk Areas</div>
              </div>
            ))}
          </div>
        </div>
            ...
            return (
              <div key={index} className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-semibold text-gray-900">{result.category}</h3>
                  ...
                  <ul className="space-y-1">
                    {result.recommendations.slice(0, 3).map((rec, i) => (
                      <li key={i} className="text-sm text-gray-600 flex items-start space-x-2">
                        <Target size={12} className="mt-1 text-blue-500 flex-shrink-0" />
                        <span>{rec}</span>
                      </li>
                    ))}
                  </ul>
